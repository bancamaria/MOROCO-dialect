...
# dtype -> daca e nr sau cuvant, sa faca diferenta
...
# make all lowercase; get n-grams only from text inside word boundaries
vector = CountVectorizer(lowercase=False, ngram_range=(4, 5), analyzer='char_wb')
# learn vocab, then make matrix
...
scaler = TfidfTransformer() # term-frequency times inverse document-frequency
normalized_training_vector = scaler.fit_transform(training_vector) # fit to data -> transform
normalized_validation_vector = scaler.transform(validation_vector)
normalized_testing_vector = scaler.transform(testing_vector) # transform matrix into tfidf repres

# Mathematical representation of a real-world process
model = ComplementNB(alpha=0.005) # aflat cu grid search
...
submission = np.vstack((testing_samples['id'], test_prediction)).T # fa 2 linii => 2 coloane
